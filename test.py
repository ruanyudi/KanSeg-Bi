...
def pred_one_epoch(opt, model, dataloader, optimizer, epoch, train=True):
    ...
    for i, (images, labels) in enumerate(dataloader):
        ...
        dataloader.set_description(
            f"Phase: {prefix} | Epoch: {epoch} | Loss: {np.mean(losses)} | MIOU: {np.mean(mious)} | {np.mean(f1_scores),np.mean(precisions),np.mean(recalls)}"
        )
...
if __name__ == "__main__":
    ...
    testLoader = torch.utils.data.DataLoader(
        testDataset, batch_size=opt.batch_size, shuffle=False
    )
