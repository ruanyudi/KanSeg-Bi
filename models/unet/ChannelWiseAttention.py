...
class ChannelWiseSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Query, Key, Value
        query = self.query_conv(x).view(batch_size, C // self.reduction, -1)  # B x (C/reduction) x (H*W)
        key = self.key_conv(x).view(batch_size, C // self.reduction, -1).permute(0, 2, 1)  # B x (H*W) x (C/reduction)
        ...
        # 加权 value
        out = torch.bmm(value.permute(0, 2, 1), attention.permute(0, 2, 1))  # B x C x (H*W)
